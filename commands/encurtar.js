...
exports.run = (client, message, args) => {
  ...
  if (!args[0]) return message.channel.send('**Como usar: +encurtar <URL> [título]**')
  ...
  if (!args[1]) { // If the second argument in the message is undefined, run this
    ...
    shorten.shorten(args[0], function(res) { // This will run the shorten function and pass it 'res'
      if (res.startsWith('Erro:')) return message.channel.send('**Sua URL é inválida!**'); // The only possible error here would be that it's not a valid URL.
